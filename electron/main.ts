...
const GITHUB_OWNER = 'GoodCompany555'
const GITHUB_REPO = 'pxvcoretalisxandriqvexorumtalinexarionzqvostrami'
...
/**
 * Запрашивает ВСЕ релизы на GitHub и находит тот,
 * у которого самая высокая версия (по semver).
 * Возвращает URL папки с ассетами этого релиза, или null если обновлений нет.
 */
async function findHighestVersionRelease(): Promise<{ url: string; version: string } | null> {
  const currentVersion = app.getVersion()
  log.info(`[AutoUpdater] Текущая версия приложения: ${currentVersion}`)

  const apiUrl = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/releases`
  log.info(`[AutoUpdater] Запрос релизов: ${apiUrl}`)

  const response = await net.fetch(apiUrl, {
    headers: { 'User-Agent': 'EasyKassa-Updater', 'Accept': 'application/vnd.github.v3+json' },
  })
  ...
  log.info(`[AutoUpdater] Найдено ${releases.length} релиз(ов) на GitHub`)
  ...
  for (const release of releases) {
    ...
    // Проверяем, что у релиза есть latest.yml
    const hasLatestYml = release.assets?.some((a: any) => a.name === 'latest.yml')
    if (!hasLatestYml) {
      log.info(`[AutoUpdater] Релиз ${release.tag_name} пропущен — нет latest.yml`)
      continue
    }

    log.info(`[AutoUpdater] Релиз ${release.tag_name} (v${version}), есть latest.yml`)

    if (compareVersions(version, bestVersion) > 0) {
      bestVersion = version
      bestTag = release.tag_name
    }
  }

  if (!bestTag) {
    log.info(`[AutoUpdater] Нет версий выше ${currentVersion}`)
    return null
  }

  const downloadBase = `https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases/download/${bestTag}`
  log.info(`[AutoUpdater] Найдена новая версия: ${bestVersion} -> ${downloadBase}`)
  return { url: downloadBase, version: bestVersion }
}

/**
 * Умная проверка обновлений:
 * 1. Запрашивает все релизы через GitHub API
 * 2. Находит релиз с самой высокой версией (не полагаясь на GitHub "latest")
 * 3. Направляет electron-updater на конкретный релиз
 */
async function smartCheckForUpdates() {
  sendUpdateEvent('updater:checking')

  try {
    const best = await findHighestVersionRelease()

    if (!best) {
      // Нет обновлений — сообщаем UI
      sendUpdateEvent('updater:not-available', { version: app.getVersion() })
      return null
    }

    // Перенаправляем autoUpdater на конкретный релиз (generic provider)
    log.info(`[AutoUpdater] Устанавливаю feedURL: ${best.url}`)
    autoUpdater.setFeedURL({
      provider: 'generic',
      url: best.url,
    })

    // Теперь autoUpdater скачает latest.yml из этого конкретного релиза
    // и корректно определит, что есть обновление
    const result = await autoUpdater.checkForUpdates()
    return result
  } catch (error: any) {
    log.error('[AutoUpdater] Ошибка умной проверки:', error.message)
...
function setupAutoUpdater() {
  ...
  // Примечание: 'checking-for-update' НЕ слушаем здесь,
  // потому что smartCheckForUpdates() отправляет 'updater:checking' сама.

  autoUpdater.on('update-available', (info) => {
    log.info(`[AutoUpdater] ✅ Найдено обновление: v${info.version} от ${info.releaseDate}`)
  ...
  autoUpdater.on('update-not-available', (info) => {
    log.info(`[AutoUpdater] На сервере: v${info.version} — обновление не требуется`)
    sendUpdateEvent('updater:not-available', { version: info.version })
  })

  autoUpdater.on('download-progress', (progress) => {
    log.info(`[AutoUpdater] Загрузка: ${Math.round(progress.percent)}%`)
  ...
  autoUpdater.on('update-downloaded', (info) => {
    log.info(`[AutoUpdater] ✅ Обновление v${info.version} скачано и готово к установке`)
  ...
  autoUpdater.on('error', (error) => {
    ...
    log.error('[AutoUpdater] Ошибка:', errorMsg)
  ...
  ipcMain.handle('updater:check', async () => {
    try {
      const result = await smartCheckForUpdates()
  ...
  setTimeout(() => {
    smartCheckForUpdates().catch((err) => {
      log.error('[AutoUpdater] Ошибка первой проверки:', err.message)
    })
  }, 5_000)
  ...
  setInterval(() => {
    smartCheckForUpdates().catch((err) => {
      log.error('[AutoUpdater] Ошибка периодической проверки:', err.message)
    })
  }, 4 * 60 * 60 * 1000)
}
