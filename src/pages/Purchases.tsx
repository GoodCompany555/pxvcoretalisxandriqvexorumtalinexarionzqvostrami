...
export default function Purchases() {
  ...
  const handleCreateSupplier = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!company?.id) return;
    if (!supplierFormData.name) return toast.error('Введите название поставщика');

    const loader = toast.loading('Сохранение...');
    try {
      let res;
      ...
      if (res.success) {
        toast.success(editingSupplierId ? 'Поставщик обновлён' : 'Поставщик добавлен', { id: loader });
      ...
      toast.error('Ошибка сохранения', { id: loader });
    }
  };
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (res.success) {
            toast.success('Поставщик удален', { id: loader });
            loadSuppliers();
            window.dispatchEvent(new Event('blur'));
            setTimeout(() => window.dispatchEvent(new Event('focus')), 50);
          } else {
            toast.error(res.error || 'Ошибка удаления', { id: loader });
          }
        } catch (e) {
  ...
  const handleCreatePurchase = async (e: React.FormEvent) => {
    ...
    if (!purchaseFormData.supplierId) return toast.error('Выберите поставщика');
    if (purchaseFormData.items.length === 0) return toast.error('Добавьте хотя бы один товар');

    const loader = toast.loading('Создание накладной...');
    try {
      ...
      if (res.success) {
        toast.success('Накладная создана (Черновик)', { id: loader });
  ...
  const handleCompletePurchase = async (id: string) => {
    ...
    setConfirmDialog({
      ...
      message: 'Вы уверены, что хотите провести эту накладную? Товары поступят на склад. Это действие необратимо.',
      onConfirm: async () => {
        setConfirmDialog(prev => ({ ...prev, isOpen: false }));
        const loader = toast.loading('Проведение накладной...');
        try {
          const res = await window.electronAPI.purchases.complete(company.id, id);
          if (res.success) {
            toast.success('Накладная проведена!', { id: loader });
            loadPurchases();
          } else {
            toast.error(res.error || 'Ошибка', { id: loader });
          }
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    setConfirmDialog({
      ...
      message: 'Отменить и удалить черновик?',
      onConfirm: async () => {
        setConfirmDialog(prev => ({ ...prev, isOpen: false }));
        const loader = toast.loading('Удаление...');
        try {
    ...
    <div className="p-8 h-full flex flex-col bg-gray-50/50">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Приходные накладные и база поставщиков</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FileText className="w-5 h-5" /> Новая приемка
          </button>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold tracking-wider">
                  ...
                  <th className="px-6 py-4 text-center">Создал</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-10 text-gray-500">Нет накладных</td></tr>
                ) : (
                  purchases.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-gray-800">{p.supplier_name || 'Не указан'}</td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${p.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                          ...
                          {p.status === 'completed' ? 'Проведена' : 'Черновик'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 flex items-center justify-center gap-3">
                        ...
                        {p.status === 'completed' && <span className="text-xs text-gray-400">Нет действий</span>}
                      </td>
                    </tr>
                  ))
                )}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold tracking-wider">
                  ...
                  <th className="px-6 py-4">БИН/ИИН</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {suppliers.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-10 text-gray-500">Нет поставщиков</td></tr>
                ) : (
                  suppliers.map((s) => (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingSupplierId ? 'Редактирование поставщика' : 'Новый поставщик'}</h2>
            ...
            <form onSubmit={handleCreateSupplier} className="p-6 flex-1 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Название/ИП <span className="text-red-500">*</span></label>
                <div className="relative">
                  <Input required value={supplierFormData.name} onChange={e => setSupplierFormData({ ...supplierFormData, name: e.target.value })} type="text" className="w-full px-3 py-2 pr-10 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none" />
                  <KeyboardIcon />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <Input value={supplierFormData.bin} onChange={e => setSupplierFormData({ ...supplierFormData, bin: e.target.value })} type="text" className="w-full px-3 py-2 pr-10 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none" />
                  <KeyboardIcon />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <Input value={supplierFormData.phone} onChange={e => setSupplierFormData({ ...supplierFormData, phone: e.target.value })} type="text" className="w-full px-3 py-2 pr-10 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none" />
                    <KeyboardIcon />
                  </div>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="relative">
                  <Input value={supplierFormData.address} onChange={e => setSupplierFormData({ ...supplierFormData, address: e.target.value })} type="text" className="w-full px-3 py-2 pr-10 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none" />
                  <KeyboardIcon />
                </div>
              </div>

              <div className="mt-6 pt-4 border-t border-gray-100 flex justify-end gap-3">
                <button type="button" onClick={() => setIsSupplierModalOpen(false)} className="px-5 py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl font-medium transition-colors">Отмена</button>
                <button type="submit" className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-xl font-medium shadow-sm transition-all active:scale-[0.98]">Сохранить</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={handleCreatePurchase} className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
              <div className="flex gap-4 items-end">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Поставщик <span className="text-red-500">*</span></label>
                  <select required value={purchaseFormData.supplierId} onChange={e => setPurchaseFormData({ ...purchaseFormData, supplierId: e.target.value })} className="w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none bg-white">
                    <option value="" disabled>Выберите поставщика...</option>
                    {suppliers.map(s => (
                      <option key={s.id} value={s.id}>{s.name} {s.bin ? `(БИН: ${s.bin})` : ''}</option>
                    ))}
                  </select>
                </div>
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Комментарий / № ТТН</label>
                  <Input value={purchaseFormData.notes} onChange={e => setPurchaseFormData({ ...purchaseFormData, notes: e.target.value })} type="text" className="w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-primary/20 outline-none" placeholder="Счет-фактура №123..." />
                </div>
              </div>

              {/* Поиск товаров */}
              <div className="border border-gray-200 rounded-xl p-4 bg-gray-50 relative">
                <label className="block text-sm font-bold text-gray-800 mb-2">Добавить товары</label>
                <div className="relative">
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                      ...
                      <th className="px-4 py-3 w-32">Кол-во</th>
                      <th className="px-4 py-3 w-40">Цена закуп. (₸)</th>
                      <th className="px-4 py-3 w-32">Сумма (₸)</th>
                      <th className="px-4 py-3 w-16 text-center"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {purchaseFormData.items.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-10 text-gray-400">Список пуст. Выполните поиск выше.</td></tr>
                    ) : (
                      purchaseFormData.items.map((item: any, i: number) => {
                        ...
                        return (
                          <tr key={item.productId} className="bg-white">
                            ...
                            <td className="px-4 py-3">
                              <Input type="number" min="0.001" step="any" value={item.quantity} onChange={e => updatePurchaseItem(i, 'quantity', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 outline-none text-center" />
                            </td>
                            <td className="px-4 py-3">
                              <Input type="number" min="0" step="any" value={item.price} onChange={e => updatePurchaseItem(i, 'price', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 outline-none text-right font-medium" />
                            </td>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-100 mt-auto">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setIsPurchaseModalOpen(false)} className="px-5 py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl font-medium transition-colors">Отмена</button>
                  <button type="submit" className="px-6 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-xl font-medium shadow-sm transition-all shadow-primary/30">Сохранить черновик</button>
                </div>
              </div>
            </form>
          </div>
