...
import { Save, Store, Link as LinkIcon, Wifi, WifiOff, Plus, Trash2, Scale, CreditCard, Globe, CheckCircle, XCircle, Eye, EyeOff, Loader2, Database, FolderOpen, RefreshCcw, Download, Upload, LayoutDashboard, AlertCircle } from 'lucide-react';
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    // Listen for update events to update status in UI
    const updater = window.electronAPI.updater;
    if (updater) {
      updater.onChecking(() => setUpdateStatus({ type: 'checking' }));
      updater.onAvailable((info: any) => setUpdateStatus({ type: 'available', serverVersion: info.version }));
      updater.onNotAvailable((info: any) => setUpdateStatus({ type: 'not-available', serverVersion: info?.version }));
      updater.onError((err: any) => setUpdateStatus({ type: 'error', message: err.message }));
    }
    if (company?.id) {
      loadTerminals();
      loadScaleSettings();
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl border border-gray-100 bg-gray-50">
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-700">Статус обновлений</p>
                <div className="mt-1 flex items-center gap-2">
                  {updateStatus.type === 'idle' && <span className="text-sm text-gray-500">Ожидание проверки</span>}
                  {updateStatus.type === 'checking' && (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin text-blue-500" />
                      <span className="text-sm text-blue-600 font-medium">Поиск новых версий...</span>
                    </>
                  )}
                  {updateStatus.type === 'available' && (
                    <>
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      <span className="text-sm text-green-600 font-medium">Найдено обновление: {updateStatus.serverVersion}</span>
                    </>
                  )}
                  {updateStatus.type === 'not-available' && (
                    <>
                      ...
                      <span className="text-sm text-gray-500 font-medium">
                        У вас актуальная версия. {updateStatus.serverVersion && `(На сервере: ${updateStatus.serverVersion})`}
                      </span>
                    </>
                  )}
                  {updateStatus.type === 'error' && (
                    <>
                      ...
                      <span className="text-sm text-red-600 font-medium">
                        {updateStatus.message || 'Ошибка проверки'}
                      </span>
                    </>
                  )}
                </div>
