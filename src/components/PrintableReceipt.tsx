      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '8px' }}>
          ...
          <div style={{ fontSize: '14px' }}>
            {receiptData.type === 'return' ? 'Возврат прихода / Кірісті қайтару' : 'Приход / Кіріс'}
          </div>
        </div>
        ...
        <div style={{ marginBottom: '8px' }}>
          {receiptData.items?.map((item: any, idx: number) => {
            const vatAmount = item.total * 16 / 116;
            return (
        ...
        <div style={{ marginBottom: '8px' }}>
          {row('Налогообложение', 'Салық салу', 'ОУР')}
          {row('Сумма НДС', 'ҚҚС сомасы 16%', `= ${Number(receiptData.vatAmount || 0).toFixed(2)}`)}
        </div>
